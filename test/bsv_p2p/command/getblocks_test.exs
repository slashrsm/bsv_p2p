defmodule BsvP2p.Command.GetBlocksTest do
  use ExUnit.Case
  alias BsvP2p.Command
  alias BsvP2p.Command.Getblocks
  doctest BsvP2p.Command.Getblocks
  ...
  test "Command.name/1" do
    assert BsvP2p.Command.name(%Getblocks{}) == "getblocks"
  end

  test "Command.get_payload/1" do
    assert Command.get_payload(%Getblocks{}) == @test_payload
  end

  test "BsvP2p.Command.Getblocks.from_payload/1" do
    assert %Getblocks{
             version: 70_015,
             locator_hashes: [
           ...
           } == Getblocks.from_payload(@test_payload)
  end

  test "BsvP2p.Command.Getblocks.set_stop_hash/2" do
    command = %Getblocks{} |> Getblocks.set_stop_hash(0)
    assert <<0::integer-size(256)-little>> == command.stop
    assert @test_payload == Command.get_payload(command)

    command =
      %Getblocks{}
      |> Getblocks.set_stop_hash(
  ...
  test "BsvP2p.Command.Getblocks.set_locator_hashes/2" do
    command =
      %Getblocks{}
      |> Getblocks.set_locator_hashes(
